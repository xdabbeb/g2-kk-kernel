...
int lgit_ois_calibration(int ver)
{
	...
	ois_i2c_e2p_write(0x090A, (uint16_t)(0xFFFF & gyro_offset_value_y), 2); //gyro_offset_value_x -> gyro_offset_value_y∑Œ ºˆ¡§«‘.(±Ë«¸∞¸)
...
int lgit2_ois_stat(struct msm_sensor_ois_info_t *ois_stat)
{
	...
	//Hall Fail Spec.
	short int spec_hall_x_lower = 1467;//+-0.65deg»Æ∫∏«œ±‚ ¿ß«œø© 45.0um « ø‰«‘.(Ω«¡¶ Spec.¿∫ +-0.5deg¿Ãπ«∑Œ √Ê∫–«— ∏∂¡¯ ∆˜«‘µ )
